// SPIFlashDMA_FileListWithImageSize (C)2014 Henning Karlsen
...
void loop()
{
  if (myFlash.ID_device!=0)
  {
    ...
    Serial.println(buf);
    Serial.println();
    Serial.println("| ID\t| Filetype\t\t\t\t| Filesize\t| ImgX\t| ImgY\t| Note");
    Serial.println("|-------|---------------------------------------|---------------|-------|-------|---------------");
    ...
    while (filetype != ERR_FILE_DOES_NOT_EXIST)
    {
      ...
      if (filetype != ERR_FILE_DOES_NOT_EXIST)
      {
        ...
        Serial.print(buf);
        Serial.print("\t| ");
        if ((filetype >=4) and (filetype<=6))
        {
          sprintf(buf, "%5d", myFlash.getImageXSize(fileid));
          Serial.print(buf);
          Serial.print(" | ");
          sprintf(buf, "%5d", myFlash.getImageYSize(fileid));
          Serial.print(buf);
          Serial.print(" | ");
        }
        else
